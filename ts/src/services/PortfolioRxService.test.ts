...
describe('PortfolioService (Unit)', () => {
  ...
  it('should fetch summary when called using injected client', async () => {
    const transformedMock = schemas.PortfolioSummary.parse(mockGetPortfolioSummarySuccessfulResponse);
  ...
  it('should update state with data from API', async () => {
    ...
    service = new PortfolioRxService({ apiBaseUrl: 'http://test', apiClient: mockClient as unknown as typeof api });

    await service.getPortfolioSummary();

    // Test synchronous getter
    expect(service.summary.balance).toBe(mockGetPortfolioSummarySuccessfulResponse.balance);
    expect(service.summary.currency).toBe(mockGetPortfolioSummarySuccessfulResponse.currency);
    expect(service.summary.change_percent).toBe(mockGetPortfolioSummarySuccessfulResponse.change_percent);
  });
  ...
  it('should handle fetch errors gracefully', async () => {
    ...
    service = new PortfolioRxService({ apiBaseUrl: 'http://test', apiClient: mockClient as unknown as typeof api });
    await expect(service.getPortfolioSummary()).rejects.toThrow('Network error');
